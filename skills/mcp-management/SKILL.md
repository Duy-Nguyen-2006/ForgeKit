...
metadata:
  author: forgekit
  version: "1.0.0"
triggers: ["MCP", "tool", "server config", "integration", "external tool", "kết nối tool"]
non_triggers: ["fix bug", "create feature", "deploy", "style UI"]
examples: ["configure MCP server", "kết nối external tool", "discover MCP capabilities"]
---
...
MCP servers configured in `.forge/.mcp.json`.

**Gemini CLI Integration** (recommended): Create symlink to `.gemini/settings.json`:
```bash
mkdir -p .gemini && ln -sf .forge/.mcp.json .gemini/settings.json
```
...
- Config loading from `.forge/.mcp.json`
...
**Method 2: Scripts**
```bash
cd .forge/skills/mcp-management/scripts && npm install
