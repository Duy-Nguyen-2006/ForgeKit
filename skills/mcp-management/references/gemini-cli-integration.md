...
Read model from `.forge/.ck.json`: `gemini.model` (default: `gemini-3-flash-preview`)
...
Gemini CLI reads MCP servers from `.gemini/settings.json`. Create a symlink to `.forge/.mcp.json`:
...
ln -sf .forge/.mcp.json .gemini/settings.json
...
1. **Configuration Loading**: Reads `.gemini/settings.json` (symlinked to `.forge/.mcp.json`)
...
Should show symlink pointing to `.forge/.mcp.json`.
