...
```bash
cd .forge/skills/mcp-management/scripts
npm install
```
...
See `.forge/.mcp.json.example` for more examples.
...
- Load config from `.forge/.mcp.json`
...
2. `.forge/skills/mcp-management/.env`
3. `.forge/skills/.env`
4. `.forge/.env`
...
The `mcp-manager` agent (`.forge/agents/mcp-manager.md`) uses this skill to:
...
Ensure `.forge/.mcp.json` exists and is valid JSON.
