...
**IMPORTANT**: Analyze skills at `.forge/skills/*` and activate as needed.
...
[ ! -f .gemini/settings.json ] && mkdir -p .gemini && ln -sf .forge/.mcp.json .gemini/settings.json
...
```bash
npx tsx .forge/skills/mcp-management/scripts/cli.ts call-tool <server> <tool> '<json-args>'
```
