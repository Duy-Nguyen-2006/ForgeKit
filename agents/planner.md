...
**IMPORTANT**: Analyze the list of skills at `.forge/skills/*` and intelligently activate the skills that are needed for the task during the process.
...
```bash
node .forge/scripts/set-active-plan.cjs {plan-dir}
```

Example:
```bash
node .forge/scripts/set-active-plan.cjs ai_docs/feature/GH-88-add-authentication
```
